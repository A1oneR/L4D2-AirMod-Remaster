...
void GetCvars_Lang() {
	...
	if (GetLanguageByCode(g_cLanguage) == -1) {
		...
		Format(g_cLanguage, sizeof(g_cLanguage), "en");
	}
}
...
void InitGameData() {
	...
	Format(g_cMapName, sizeof(g_cMapName), "Initializing");
...
bool InitKvFile() {
	...
	if (!FileExists(kvPath)) {
		...
		// 写出默认值内容
		kv.SetString("description", "Left 8 Dead 4");
		kv.SetString("inFinale", "Activated");
		kv.SetString("notInFinale", "NotActive");
	...
	// 获取Kv文本内信息写入变量中
	kv.GetString("description", g_cGameDes, sizeof(g_cGameDes), "Left 8 Dead 4");
	kv.GetString("inFinale", g_cInFinale, sizeof(g_cInFinale), "Activated");
	kv.GetString("notInFinale", g_cNotInFinale, sizeof(g_cNotInFinale), "NotActive");
