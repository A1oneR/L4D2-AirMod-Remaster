			...
			else if (afkKickEnabled)  // if player is on spectators and kick on spectators is enabled ...
			{
				...
				for (int client = 1; client <= MaxClients; client++)
				{
					if (IsClientInGame(client))
					{
						...
						if ((!IsPlayerAlive(client) || IsIncapacitated(client)) && GetClientTeam(client) == L4D2Team_Survivor && IsFakeClient(client))
						{
							survLimit--;
						}
						if (IsFakeClient(client))
				...
				if (((survReadyCount + infReadyCount) < survLimit + zombLimit) && !IsInPause()) // In game players < ShouldBe
				{
...
void afkKickClient (int client)
{
	...
	PrintToChatAll("\x01\x04[SG] \x03%s \x01旁觀閒置太久被踢出伺服器.", PlayerName);
}
