...
void SendPrivateChat(client, target, const char[] message)
{
	if (client == 0)
	{
		if (csgo)
			...
			CPrintToChat(target, "{lightgreen}({olive}Console {default}to {olive}You{lightgreen}) {default}Console :  {default}%s", message);
	}
	...
	if (target != client)
	{
		if (csgo)
			...
			CPrintToChatEx(client, client, "{lightgreen}({olive}You {default}to {olive}%N{lightgreen}) {olive}%N {default}:  {default}%s", target, client, message);
	}
	
	if (csgo)
		PrintToChat(target, "(%N to You) %N :  %s", client, client, message);
	else
		CPrintToChatEx(target, client, "{lightgreen}({olive}%N {default}to {olive}You{lightgreen}) {olive}%N {default}:  {default}%s", client, client, message);
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (CheckCommandAccess(i, "sm_pmspy", ADMFLAG_KICK, false) && pmspy[i] && i != client && i != target && CanUserTarget(i, client))
		{
			if (csgo)
				...
				CPrintToChatEx(i, client, "{green}[PM SPY] {lightgreen}({olive}%N {default}to {olive}%N{lightgreen}) {olive}%N {default}:  {default}%s", client, target, client, message);
		}
	}
...
{
	if (StrStarts(sArgs, "!w ") || StrStarts(sArgs, "!p ") || StrStarts(sArgs, "!qr ") || StrStarts(sArgs, "!reply ")) return Plugin_Handled;
