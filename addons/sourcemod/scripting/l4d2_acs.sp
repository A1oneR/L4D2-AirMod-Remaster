...
public void OnPluginStart() {
	...
	g_hCVar_VotingAdDelayTime = CreateConVar("l4d2_acs_voting_ad_delay_time", "10.0", "设置救援关幸存者离开开始区域多少秒后执行下一张地图的更换方式.", FCVAR_NOTIFY);
	g_hCVar_NextMapMenuOptions = CreateConVar("l4d2_acs_next_map_menu_options", "2", "设置救援关投票换图列表里地图的显示类型. 0=官方和附加地图, 1=仅限官方图, 2=当前地图的类型.", FCVAR_NOTIFY);
	g_hCVar_NextMapMenuExcludes = CreateConVar("l4d2_acs_next_map_menu_excludes", "1", "设置救援关投票换图列表里排除当前地图? 0=不排除, 1=排除当前地图.", FCVAR_NOTIFY);
	g_hCVar_NextMapMenuOrder= CreateConVar("l4d2_acs_next_map_menu_order", "0", "设置救援关投票菜单中地图的显示顺序. 0=官方图,然后是附加地图, 1=随机.", FCVAR_NOTIFY);
	g_hCVar_NextMapAdMode = CreateConVar("l4d2_acs_next_map_ad_mode", "2", "设置救援关公告下一张地图信息的显示方式. 0=禁用, 1=屏幕中下, 2=聊天窗.", FCVAR_NOTIFY);
	g_hCVar_NextMapAdInterval = CreateConVar("l4d2_acs_next_map_ad_interval", "60.0", "设置救援关公告下一张地图信息的时间(秒).", FCVAR_NOTIFY);
	g_hCVar_MaxFinaleFailures = CreateConVar("l4d2_acs_max_coop_finale_failures", "1", "设置战役模式救援关幸存者团灭多少次后自动换图? 0=禁用.", FCVAR_NOTIFY);
	g_hCVar_ChMapPolicy =  CreateConVar("l4d2_acs_chmap_policy", "2", " 启用 !chmap 和 !chmap2 投票换图指令?\n 0=禁用(同时禁用公告投票换图指令).\n 1=启用(不投票视为不同意). \n 2=启用(不投票视为同意)", FCVAR_NOTIFY);	
	g_hCVar_ChMapBroadcastInterval =  CreateConVar("l4d2_acs_chmap_broadcast_interval", "180.0", "设置循环公告 \"!chmap\" 指令的时间(秒). 0=禁用.", FCVAR_NOTIFY);	
...
void DisplayNextMapTo_Scavenge(int client, bool replyCMD) {
	...
	if(g_iWinningMapVotes > 0) {
		...
		if (replyCMD) {
			Format(colorizedName, sizeof(colorizedName), "\x03%s\x05", localizedName);
			ReplyToCommand(client, "\x04[提示]%t", "The next map is currently", colorizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) {
			PrintHintText(client, "%t", "The next map is currently", localizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_CHAT) {
			Format(colorizedName, sizeof(colorizedName), "\x03%s\x05", localizedName);
			PrintToChat(client, "\x04[提示]%t", "The next map is currently", colorizedName);
		}
	}
}
