	...
	if (strcmp(classname, "infected") == 0 && IsInfiniteHordeActive()) {
		...
		if (commonTotal >= commonLimit) {
			if (!announcedEventEnd){
				CPrintToChatAll("<{olive}尸潮{default}> {red}无 {default}剩余!");
		...
		if (hCvarHordeCheckpointAnnounce.BoolValue && 
			...
			if (remaining != 0) {
				CPrintToChatAll("<{olive}尸潮{default}> {red}%i {default}剩余..", remaining);
			}
...
public Action L4D_OnSpawnMob(int &amount)
{
	...
	if (IsInfiniteHordeActive()) {
		if (!announcedInChat) {
			CPrintToChatAll("<{olive}尸潮{default}> 一个 {blue}有限尸潮事件已触发，{default}约有 {olive}%i{default}只丧尸! 冲锋或防守任君选择!", commonLimit);
