...
ConVar sb_fix_com_shotgun_range     = null;
ConVar sb_fix_com_sniper_range      = null;
ConVar sb_fix_com_shotgun_m2        = null;
ConVar sb_fix_com_change_melee      = null;
ConVar sb_fix_com_melee_range       = null;
...
float c_fCom_ShotgunRange;
int c_iCom_ShotgunM2;
float c_fCom_SniperRange;
bool c_bCom_ChangeMelee;
float c_fCom_MeleeRange;
...
float IgnoreTimer[MAXPLAYERS + 1];
bool bSpecial[MAXPLAYERS + 1];
int g_iShotgunShove[MAXPLAYERS + 1];
...
public void OnPluginStart()
{
	...
	sb_fix_help_range				= CreateConVar("sb_fix_help_range", "1000", "Range to shoot/search a pinning survivor. <1 ~ 3000 | def: 1200>", FCVAR_NOTIFY, true, 1.0, true, 3000.0);
	...
	sb_fix_ci_range				= CreateConVar("sb_fix_ci_range", "70", "Range to shoot/search a Common Infected. <1 ~ 2000 | def: 500>", FCVAR_NOTIFY, true, 1.0, true, 2000.0);
	...
	sb_fix_ci_melee_range			= CreateConVar("sb_fix_ci_melee_range", "100", "If \"sb_fix_ci_melee_allow\" is enabled, range to deal with the melee weapon. <1 ~ 500 | def: 160>", FCVAR_NOTIFY, true, 1.0, true, 500.0);
	// ---------------------------------
	...
	sb_fix_si_range				= CreateConVar("sb_fix_si_range", "200", "Range to shoot/search a Special Infected. <1 ~ 3000 | def: 500>", FCVAR_NOTIFY, true, 1.0, true, 3000.0);
	...
	// ---------------------------------
	sb_fix_com_shotgun_range        = CreateConVar("sb_fix_com_shotgun_range", "175", "Shotgun Range that AI should shot.", FCVAR_NOTIFY, true, 1.0, true, 1000.0);
	sb_fix_com_shotgun_m2           = CreateConVar("sb_fix_com_shotgun_m2", "2", "Do a m2 in this times when shot a buck.", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	sb_fix_com_sniper_range         = CreateConVar("sb_fix_com_sniper_range", "750", "Sniper Range that AI should shot.", FCVAR_NOTIFY, true, 1.0, true, 10000.0);
	sb_fix_com_change_melee         = CreateConVar("sb_fix_com_change_melee", "1", "Will AI switch melee when a SI is in range.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	sb_fix_com_melee_range          = CreateConVar("sb_fix_com_melee_range", "90", "Switch the melee when A SI in this range", FCVAR_NOTIFY, true, 1.0, true, 500.0);
	// ---------------------------------
	...
	sb_fix_tank_range				= CreateConVar("sb_fix_tank_range", "2000", "Range to shoot/search a Tank. <1 ~ 3000 | def: 1200>", FCVAR_NOTIFY, true, 1.0, true, 3000.0);
	// ---------------------------------
	...
	sb_fix_bash_hunter_chance		= CreateConVar("sb_fix_bash_hunter_chance", "0", "Chance of bash a flying Hunter. (Even 100 doesn't can perfectly shove). <1 ~ 100 | def: 100>", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	...
	sb_fix_bash_jockey_chance		= CreateConVar("sb_fix_bash_jockey_chance", "50", "Chance of bash a flying Jockey. (Even 100 doesn't can perfectly shove). <1 ~ 100 | def: 100>", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	...
	sb_fix_rock_range				= CreateConVar("sb_fix_rock_range", "800", "Range to shoot/search a tank rock. <1 ~ 2000 | def: 700>", FCVAR_NOTIFY, true, 1.0, true, 2000.0);
	// ---------------------------------
	...
	hCvarTimeToIgnore = CreateConVar("l4d2_time_to_ignore", "3.0", "How many time should the SI be ignored.");
	...
	// ---------------------------------
	sb_fix_com_shotgun_range.AddChangeHook(SBCOM_ChangeConvar);
	sb_fix_com_shotgun_m2.AddChangeHook(SBCOM_ChangeConvar);
	sb_fix_com_sniper_range.AddChangeHook(SBCOM_ChangeConvar);
	sb_fix_com_change_melee.AddChangeHook(SBCOM_ChangeConvar);
	sb_fix_com_melee_range.AddChangeHook(SBCOM_ChangeConvar);
	// ---------------------------------
...
public void SBCOM_ChangeConvar(Handle convar, const char[] oldValue, const char[] newValue) { input_COM(); }
...
void input_COM()
{
	c_fCom_ShotgunRange = GetConVarInt(sb_fix_com_shotgun_range) * 1.0;
	c_fCom_SniperRange = GetConVarInt(sb_fix_com_sniper_range) * 1.0;
	c_iCom_ShotgunM2 = GetConVarInt(sb_fix_com_shotgun_m2);
	c_bCom_ChangeMelee = GetConVarBool(sb_fix_com_change_melee);
	c_fCom_MeleeRange = GetConVarInt(sb_fix_com_melee_range) * 1.0;
}
...
{
    TimeToIgnore = GetConVarFloat(hCvarTimeToIgnore);
}
...
public Action Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
		...
		IgnoreTimer[client] = TimeToIgnore;
		bSpecial[client] = false;
		if (GetClientTeam(client) == 3 && IsPlayerTank(client) == false)
		{
	        CreateTimer(0.5,ForceIgnore,GetClientUserId(client),TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
		}
}

public Action ForceIgnore(Handle timer,int userid)
{
	...
	if (IsPlayerTank(Client) == true)
	{
		...
		IgnoreTimer[Client] = 0.0;
		return Plugin_Stop;
	}
	IgnoreTimer[Client] = IgnoreTimer[Client] - 0.5;
	if (IgnoreTimer[Client] <= 0)
	{
		IgnoreTimer[Client] = 0.0;
		return Plugin_Stop;
	}
...
stock Action onSBRunCmd(int client, int &buttons, float vel[3], float angles[3])
{
		...
		&& GetEntityMoveType(client) != MOVETYPE_LADDER)
	{
		...
		*
		==================================================================================================== */ 
		if (new_target > 0 && IgnoreTimer[new_target] <= 0.0) {
			float c_pos[3], e_pos[3];
			float lookat[3];
