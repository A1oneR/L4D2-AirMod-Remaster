...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {blue}近战 {default}击倒了 {olive}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {blue}近战 {default}击倒了 {olive}charger", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}近战击倒了 {olive}%N {default}({blue}%i 伤害{default})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}近战击倒了 {default}charger ({blue}%i 伤害{default})", attacker, damage );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {blue}砍断了 {olive}%N{default}的舌头（距离: {blue}%.1f米{default}）", attacker, victim, final);
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {blue}砍断了 {default}舌头（距离: {blue}%.1f米{default}）", attacker, final);
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)) 
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}从 {olive}%N{default}的舌头手中{blue}%s{olive}自救了", attacker, victim, (withShove) ? " 通过推" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}从舌头手中{blue}%s{olive}自救了", attacker, (withShove) ? "通过推" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {olive}%N {blue}打爆了 {default}Tank的石头", attacker );
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {red}高扑了 {olive}%N {default}({red}高度{default}: {red}%i{default})", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{green}★ {olive}%N {default}从 {olive}%N{default}的 %s 手中{blue}秒救了 {olive}%N({blue}%.2f {default}秒)",
                            attacker, victim,
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    CPrintToChatAll( "{green}★ {olive}%N {default}从 %s 手中{blue}秒救了 {olive}%N ({blue}%.2f {default}秒)",
                            attacker,
                            g_csSIClassName[zombieClass], pinVictim,
                            fClearTime
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        CPrintToChatAll("{green}★ {olive}%N {default}完成了 {blue}%i 次连跳 ({blue}最高速度: {olive}%.1f{default})",
				survivor,
                streak,
                maxVelocity
            );
