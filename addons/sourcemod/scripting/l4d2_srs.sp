...
public void OnPluginStart()
{
	...
	g_hCvar_sound_join  = CreateConVar("srs_sound_join", "0", "Join sound?\n0=Off, 1=On", CVAR_FLAGS);
	...
	g_hCvar_extra_ci_spawn_time = CreateConVar("srs_extra_ci_spawn_time", "-1", "Extra common intected spawn time.\n-1=OFF,0=Depends on amount of players (60 - (survivors*3), min=30.0)", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_S_K_CI_HS] = CreateConVar("srs_s_k_ci_hs_score", "1.5", "How many score for killing a common infected by headshot? (Survivor)", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_SK_HS] = CreateConVar("srs_s_k_sk_hs_score", "5.0", "How many score for killing a Smoker? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_B_HS]  = CreateConVar("srs_s_k_b_hs_score", "5.0", "How many score for killing a Boomer? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_H_HS]  = CreateConVar("srs_s_k_h_hs_score", "5.0", "How many score for killing a Hunter? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_ST_HS] = CreateConVar("srs_s_k_st_hs_score", "5.0", "How many score for killing a Spitter? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_J_HS]  = CreateConVar("srs_s_k_j_hs_score", "5.0", "How many score for killing a Jockey? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_C_HS]  = CreateConVar("srs_s_k_c_hs_score", "5.0", "How many score for killing a Changer? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_W_HS]  = CreateConVar("srs_s_k_w_hs_score", "12.0", "How many score for killing a Witch? (Survivor) by headshot", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_T_HS]  = CreateConVar("srs_s_k_t_hs_score", "40.0", "How many score for killing a Tank? (Survivor) by headshot", CVAR_FLAGS);

	g_hCvar_scores[SRS_S_K_W_OS] = CreateConVar("srs_s_k_w_os_score", "10.0", "How many score for killing a Witch in one shot? (Survivor)", CVAR_FLAGS);

	g_hCvar_scores[SRS_S_K_SMG]      = CreateConVar("srs_s_k_smg", "1.0", "How many score multiply/divide for killing a ci/si by smg?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_SILENCED] = CreateConVar("srs_s_k_silenced", "1.0", "How many score multiply/divide for killing a ci/si by SMG silenced?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_MP5]      = CreateConVar("srs_s_k_mp5", "1.0", "How many score multiply/divide for killing a ci/si by mp5?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_S_K_AK47]     = CreateConVar("srs_s_k_ak47", "1.0", "How many score multiply/divide for killing a ci/si by ak47?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_SG552]    = CreateConVar("srs_s_k_sg552", "1.0", "How many score multiply/divide for killing a ci/si by Sg552?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_S_K_SCOUT]    = CreateConVar("srs_s_k_scout", "1.5", "How many score multiply/divide for killing a ci/si by Scout?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_S_K_MAGNUM]   = CreateConVar("srs_s_k_magnum", "1.0", "How many score multiply/divide for killing a ci/si by magnum?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	g_hCvar_scores[SRS_S_K_GL]       = CreateConVar("srs_s_k_gl", "1.0", "How many score multiply/divide for killing a ci/si by Grenade Launcher?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_S_K_PITCHFORK] = CreateConVar("srs_s_k_pitchfork", "1.2", "How many score multiply/divide for killing a ci/si by pitchfork?\n1.0=No Multiply/Divide", CVAR_FLAGS);
	...
	g_hCvar_scores[SRS_I_DMG] = CreateConVar("srs_i_dmg_score", "1.2", "How many score for SI damage", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_KILL] = CreateConVar("srs_i_kill_score", "36.8", "How many score for SI Kill survivor", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_SK_POUNCE] = CreateConVar("srs_i_s_score", "2.4", "How many score for Smoker grab", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_C_PUMMEL] = CreateConVar("srs_i_c_score", "3.2", "How many score for Charger Stummble", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_J_RIDE] = CreateConVar("srs_i_j_score", "2.8", "How many score for Jockey ride", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_H_PUNCHED] = CreateConVar("srs_i_h_score", "2.8", "How many score for Hunter Pounch", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_INCAPACITATE] = CreateConVar("srs_i_incap_score", "12.8", "How many score for incap a survivor", CVAR_FLAGS);
	g_hCvar_scores[SRS_I_B_NOW_IT] = CreateConVar("srs_i_b_score", "2.4", "How many score for boomer vomit", CVAR_FLAGS);
...
public Action Event_PlayerIncapacitated(Event event, char[] event_name, bool dontBroadcast)
{
	...
	if (IsInfected(attacker) && IsValidClient(attacker)) 
	{
		CallUpdateAgent(g_players[attacker], SRS_I_INCAPACITATE, 1, false);
		UpdateScore(g_players[attacker], SRS_I_INCAPACITATE, 0, false);
	}
...
public Action Event_BoomerVomit(Event event, char[] event_name, bool dontBroadcast)
{
	...
	CallUpdateAgent(g_players[client], SRS_I_B_NOW_IT, 1, false);
	UpdateScore(g_players[client], SRS_I_B_NOW_IT, 0, false);
...
public Action Event_LungePounce(Event event, char[] event_name, bool dontBroadcast)
{
	// TODO: uipdate SRS data
	int client = GetClientOfUserId( GetEventInt(event, "userid") );
	CallUpdateAgent(g_players[client], SRS_I_H_PUNCHED, 1, false);
	UpdateScore(g_players[client], SRS_I_H_PUNCHED, 0, false);
...
public Action Event_TongueGrab(Event event, char[] event_name, bool dontBroadcast)
{
	// TODO: uipdate SRS data
	int client = GetClientOfUserId( GetEventInt(event, "userid") );
	CallUpdateAgent(g_players[client], SRS_I_SK_POUNCE, 1, false);
	UpdateScore(g_players[client], SRS_I_SK_POUNCE, 0, false);
...
public Action Event_JockeyRide(Event event, char[] event_name, bool dontBroadcast)
{
	// TODO: uipdate SRS data
	int client = GetClientOfUserId( GetEventInt(event, "userid") );
	CallUpdateAgent(g_players[client], SRS_I_J_RIDE, 1, false);
	UpdateScore(g_players[client], SRS_I_J_RIDE, 0, false);
...
public Action Event_ChargerImpact(Event event, char[] event_name, bool dontBroadcast)
{
	// TODO: uipdate SRS data
	int client = GetClientOfUserId( GetEventInt(event, "userid") );
	CallUpdateAgent(g_players[client], SRS_I_C_CARRY, 1, false);
...
public Action Event_ChargerPummel(Event event, char[] event_name, bool dontBroadcast)
{
	// TODO: uipdate SRS data
	int client = GetClientOfUserId( GetEventInt(event, "userid") );
	CallUpdateAgent(g_players[client], SRS_I_C_PUMMEL, 1, false);
	UpdateScore(g_players[client], SRS_I_C_PUMMEL, 0, false);
...
public Action Event_PlayerDeath(Event event, char[] event_name, bool dontBroadcast)
{
	...
	else if (IsInfected(killer) && IsSurvivor(victim))
	{
		...
		UpdateScore(g_players[killer], SRS_I_KILL, 0, false);
	}
	return Plugin_Handled;
}
...
public Action Event_PlayerHurt(Event event, char[] event_name, bool dontBroadcast)
{
	...
	if (IsSurvivor(victim) && IsInfected(attacker))
	{
		//To Do later
		//if (g_players[attacker].weapon.IsGunOrMelee()) CallUpdateAgent(g_players[attacker], SRS_S_HIT, 1, false);
		CallUpdateAgent(g_players[attacker], SRS_I_DMG, damage, false);
		UpdateScore(g_players[attacker], SRS_I_DMG * damage, 0, false);
	}
...
public Action CP_OnChatMessage(int& author, ArrayList recipients, char[] flagstring, char[] name, char[] message, bool& processcolors, bool& removecolors)
{
	...
	//Format(message, 128, "{default}%s", message);
	{
		...
		//IntToString(GetClientRating(author), sPoints, sizeof(sPoints));
		char sNewName[MAXLENGTH_NAME];
		char sNewMessage[MAXLENGTH_MESSAGE];
		float sPoints;
		int srsType = IsFakeClient(author) ? SRS_TYPE_MVP_BOT : SRS_TYPE_MVP;
		sPoints = GetScore(g_players[author], srsType);
		if (sPoints <= 0)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "初来乍到");
		}
		else if (sPoints > 0 && sPoints < 2000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "炼气初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "炼气初期");
		}
		else if (sPoints >= 2000 && sPoints < 4000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "炼气中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "炼气中期");
		}
		else if (sPoints >= 4000 && sPoints < 6000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "炼气后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "炼气后期");
		}
		else if (sPoints >= 6000 && sPoints < 10000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "炼气圆满");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "炼气圆满");
		}
		else if (sPoints >= 10000 && sPoints < 13000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "筑基初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "筑基初期");
		}
		else if (sPoints >= 13000 && sPoints < 16000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "筑基中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "筑基中期");
		}
		else if (sPoints >= 16000 && sPoints < 19000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "筑基后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "筑基后期");
		}
		else if (sPoints >= 19000 && sPoints < 25000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "筑基圆满");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "筑基圆满");
		}
		else if (sPoints >= 25000 && sPoints < 30000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "金丹初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "金丹初期");
		}
		else if (sPoints >= 30000 && sPoints < 35000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "金丹中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "金丹中期");
		}
		else if (sPoints >= 35000 && sPoints < 40000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "金丹后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "金丹后期");
		}
		else if (sPoints >= 40000 && sPoints < 48000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "金丹圆满");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "金丹圆满");
		}
		else if (sPoints >= 48000 && sPoints < 56000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "元婴初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "元婴初期");
		}
		else if (sPoints >= 56000 && sPoints < 64000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "元婴中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "元婴中期");
		}
		else if (sPoints >= 64000 && sPoints < 76000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "元婴后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "元婴后期");
		}
		else if (sPoints >= 76000 && sPoints < 91000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "化神初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "化神初期");
		}
		else if (sPoints >= 91000 && sPoints < 106000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "化神中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "化神中期");
		}
		else if (sPoints >= 106000 && sPoints < 136000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "化神后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "化神后期");
		}
		else if (sPoints >= 136000 && sPoints < 186000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "合体初期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "合体初期");
		}
		else if (sPoints >= 186000 && sPoints < 236000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "合体中期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "合体中期");
		}
		else if (sPoints >= 236000 && sPoints < 300000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "合体后期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "合体后期");
		}
		else if (sPoints >= 300000 && sPoints < 500000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "渡劫期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "渡劫期");
		}
		else if (sPoints >= 500000 && sPoints < 1000000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "大乘期");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "大乘期");
		}
		else if (sPoints >= 1000000)
		{
			ReplaceString(sNewName, sizeof(sNewName), "{score}", "金仙");
			ReplaceString(sNewMessage, sizeof(sNewMessage), "{score}", "金仙");
		}
	}
