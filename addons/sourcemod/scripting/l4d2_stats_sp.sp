...
public void Event_PlayerDeath(Event hEvent, const char[] sEventName, bool bDontBroadcast)
{
	...
	if (GetClientTeam(attacker) == TEAM_SURVIVOR && GetClientTeam(victim) == TEAM_INFECTED) {
		...
		if (zombieclass == L4D2Infected_Boomer) {
			...
			GetClientAbsOrigin(attacker, AttackerPosition[attacker]);
			GetClientAbsOrigin(victim, VictimPosition[victim]);
			float xdistance = FloatAbs(AttackerPosition[attacker][0] - VictimPosition[victim][0]);
			float ydistance = FloatAbs(AttackerPosition[attacker][1] - VictimPosition[victim][1]);
			float zdistance = FloatAbs(AttackerPosition[attacker][2] - VictimPosition[victim][2]);
			float fdistance = SquareRoot(Pow(xdistance, 2.0) + Pow(ydistance, 2.0) + Pow(zdistance, 2.0));
			float final = fdistance * 0.0213;
			...
			if (strcmp(weapon, "weapon_smg") == 0)
			{
				chweapon = "伍兹冲锋枪";
			}
			if (strcmp(weapon, "weapon_smg_silenced") == 0)
			{
				chweapon = "马克冲锋枪";
			}
			if (strcmp(weapon, "weapon_smg_mp5") == 0)
			{
				chweapon = "MP5冲锋枪";
			}
			if (strcmp(weapon, "weapon_pistol") == 0)
			{
				chweapon = "手枪";
			}
			if (strcmp(weapon, "weapon_pistol_magnum") == 0)
			{
				chweapon = "沙鹰";
			}
			if (strcmp(weapon, "weapon_pumpshotgun") == 0)
			{
				chweapon = "木喷";
			}
			if (strcmp(weapon, "weapon_shotgun_chrome") == 0)
			{
				chweapon = "铁喷";
			}
			if (strcmp(weapon, "weapon_sniper_scout") == 0)
			{
				chweapon = "Scout狙击枪";
			}
			if (strcmp(weapon, "weapon_rifle") == 0)
			{
				chweapon = "M16步枪";
			}
			if (strcmp(weapon, "weapon_rifle_ak47") == 0)
			{
				chweapon = "AK步枪";
			}
			if (strcmp(weapon, "weapon_rifle_sg552") == 0)
			{
				chweapon = "SG步枪";
			}
			if (strcmp(weapon, "weapon_rifle_desert") == 0)
			{
				chweapon = "SCAR步枪";
			}
			if (strcmp(weapon, "weapon_hunting_rifle") == 0)
			{
				chweapon = "M14狙击枪";
			}
			if (strcmp(weapon, "weapon_sniper_military") == 0)
			{
				chweapon = "G3狙击枪";
			}
			if (strcmp(weapon, "weapon_sniper_awp") == 0)
			{
				chweapon = "AWP狙击枪";
			}
			if (strcmp(weapon, "weapon_autoshotgun") == 0)
			{
				chweapon = "M1014霰弹枪";
			}
			if (strcmp(weapon, "weapon_shotgun_spas") == 0)
			{
				chweapon = "SPAS霰弹枪";
			}
			...
			if (strcmp(weapon, "weapon_melee") == 0) {
				...
				CPrintToChatAll("{green}★ {olive}%N {blue}近战{default}空爆了 {olive}%N", attacker, victim);
				...
				for (int b = 1; b <= MaxClients; b++) {
					//Print to Specs!
					if (IsClientInGame(b) && (victim != b) && (attacker != b)) {
						CPrintToChat(b, "{green}★ {olive}%N {default}用 {blue}%d {default}枪{default}空爆了 {olive}%N{default} （{olive}距离{default}: {blue}%.1f{default}米, {olive}武器{default}: {blue}%s{default}）", \
															attacker, shots, victim, final, chweapon);
					}
				}*/
				...
				CPrintToChatAll("{green}★ {olive}%N {default}空爆了 {olive}%N {default}造成了 {blue}%d 伤害 {default}在 {blue}%d {default}枪内. 助攻: {olive}%s", \
															attacker, victim, damage, shots, assister_string);
			} else {
				...
				CPrintToChatAll("{green}★ {olive}%N {default}用 {blue}%d {default}枪{default}空爆了 {olive}%N{default} （{olive}距离{default}: {blue}%.1f{default}米, {olive}武器{default}: {blue}%s{default}）", \
															attacker, shots, victim, final, chweapon);
			}
		}
	}
...
public Action Timer_BoomerKilledCheck(Handle hTimer)
{
	...
	if (IsValidClient(g_iBoomerKiller)) {
		if (IsValidClient(g_iBoomerClient)) {
			...
			CPrintToChatAll("{green}%s {olive}%N {default}打爆了 {olive}%s{default}的Boomer {blue}%0.1f秒", sRank, g_iBoomerKiller, Boomer, BoomerKillTime);
		}
	}
	////
