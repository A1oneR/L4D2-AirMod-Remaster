...
public void PrintTankSkill()
{
	...
	FormatEx(info, sizeof(info), "[{green}!{default}] {blue}Tank {default}伤害报告({olive}%s{default})", name);
	dp.WriteString(info);
	FormatEx(info, sizeof(info), "{green}> {default}拳击:  {red}%i {green}/ {default}丢饼:  {red}%i {green}/ {default}打铁:  {red}%i", g_eTankAttack.Punch, g_eTankAttack.Rock, g_eTankAttack.Hittable);
	dp.WriteString(info);
	FormatEx(info, sizeof(info), "{green}> {blue}生还者 {default}倒地:  {olive}%i {green}/ {default}死亡:  {olive}%i ", g_eTankResult.Incap, g_eTankResult.Death);
	dp.WriteString(info);
	FormatEx(info, sizeof(info), "{green}> {default}时长:  {lightgreen}%s {green}/ {default}共计伤害:  {lightgreen}%i", buffer, g_eTankResult.TotalDamage);
	dp.WriteString(info);
}
