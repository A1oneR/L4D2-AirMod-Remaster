...
public PlayerDied_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsClientAndInGame(victim) && GetClientTeam(victim) == TEAM_SURVIVOR && IsWitch(attacker) && Witchkilled)
	{
		GiveWitchBonus();
		WitchReset();
        Witchkilled = false;
...
{
	iAttack = 0;
    CreateTimer(0.5, CacWitchKilled, _, TIMER_FLAG_NO_MAPCHANGE);
}

public Action:CacWitchKilled(Handle:timer) 
{
	Witchkilled = true;
}
