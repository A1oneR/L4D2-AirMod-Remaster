...
public OnTankRockSkeeted(int survivor, int tank)
{
	g_iRock[survivor] += 1;
}
...
public Event_PlayerKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CreateTimer(0.3, Timer_CheckTank, victim); // Use a delayed timer due to bugs where the tank passes to another player
}
...
PrintTankDamage()
{
	...
	new survivor_clients[g_iSurvivorLimit + 1]; // Array to store survivor client indexes in, for the display iteration
	decl percent_damage, damage, rock;
	...
	for (new k; k <= survivor_index; k++)
	{
		...
		CPrintToChatAll("{olive}%N {blue}[{default}%d{blue}] [{default}%d{blue} Rock]({default}%i%%{blue})", client, damage, rock, percent_damage);
	}
}
