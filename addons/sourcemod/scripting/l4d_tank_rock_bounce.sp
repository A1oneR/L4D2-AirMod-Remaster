...
{
	g_hCvarAllow =		CreateConVar(	"l4d_tank_rock_bounce_allow",		"0",			"0=Plugin off, 1=Plugin on.", CVAR_FLAGS );
	...
	g_hCvarRepeat =		CreateConVar(	"l4d_tank_rock_bounce_repeat",		"10",			"Number of times a single rock is allowed to bounce before exploding.", CVAR_FLAGS );
	g_hCvarScale =		CreateConVar(	"l4d_tank_rock_bounce_scale",		"0.99",			"Each bounce will scale the velocity by this much. 1.0 = Bounce at full speed. 0.5 = Half velocity each bounce. 0.2 = Very slow after bounce.", CVAR_FLAGS );
	g_hCvarSpeed =		CreateConVar(	"l4d_tank_rock_bounce_speed",		"300.0",		"Minimum speed the rock must be moving to allow bouncing.", CVAR_FLAGS );
